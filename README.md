...
**Live: [trailstages.com](https://trailstages.com)**
...
**No backend, no accounts, no build step.** This repo is the static runtime
only: `docs/` (plain HTML/CSS/JS) loads the pre-baked JSON in
`docs/data/whw/`, runs the stage-segmentation algorithm in the browser, and
renders a Leaflet map + itinerary panel. The only network calls at runtime
are map tiles and outbound Booking.com links.

The JSON in `docs/data/whw/` is generated by a separate Python build
pipeline (OSM data via the Overpass API → stitched route → accommodation
projected onto it) that isn't part of this repo. The segmentation engine
(`docs/planner.js`) is a pure function — trail data in, itinerary out — with
...
fail because browsers block `fetch()` of local JSON under the file
protocol. Not required to use the live site above; only useful for local
development.
...
order (`docs/planner.js`'s `buildAccommodationLink`):
